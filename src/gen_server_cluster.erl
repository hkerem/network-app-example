...
%% Starts a local server if a global server is already running in a remote node,
%% if there is a any other cluster instance in the same node, ignore is returned
...
start_local_server(Name) ->
	case is_running(Name) of
		...
		remote ->
			%% if there is an other local instance, return ignore
...
stop(Name, all) ->
	...
	case LocalServerNodeList of
		[] -> ok;
		_ -> gen_server:multi_call(LocalServerNodeList, Name, Request)
	end,
...
handle_call(Request, From, State) ->
	delegate_to_target(State, handle_call, [Request, From, State#state.targetState]).
...
handle_cast({unlink,Pid}, State) ->
	...
	RemoveTargetLinkedPidFun = fun(S) ->
